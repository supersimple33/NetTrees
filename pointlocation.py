...
class ParallelPointLocation(PointLocation):
    ...
    def nn(self, point):
        ...
        if dist(child, point) > self.tree.cr * self.tree._taupow(child.level):
    ...
    def nnhelper(self, point, currentnodes, level):
        ...
        if len(currentnodes) == 0:
            return None
        if point.distto(*[n.point for n in currentnodes]) > self.tree.cr * self.tree._taupow(level):
            return None
        children = ch(currentnodes)
        nextlevel = max(n.level for n in children)
        nextthreshold = self.tree.cr * self.tree._taupow(nextlevel)
        nextnodes = {n if n.level == nextlevel else n.par 
                     for n in children if dist(n, point) <= nextthreshold}
...
class SinglePathPointLocation(PointLocation):
    ...
    def nn(self, point):
        ...
        self.basictouchno += 1
        while dist(nextnode, point) <= self.tree.cr * self.tree._taupow(nextnode.level):
            currentnode = nextnode
            allnodes = ch(rel(currentnode))
            nextlevel = max(n.level for n in allnodes)
            nextnode = self.minnodewithcoveringdistance(allnodes, point, nextlevel)
        return currentnode

    def minnodewithcoveringdistance(self, nodes, point, level):
        threshold = self.tree.cr * self.tree._taupow(level)
        bestnode = None
        bestdist = float('inf')
        fallback = None
        for node in nodes:
            ...
            dst = dist(node, point)
            self.basictouchno += 1
            if dst <= threshold and dst < bestdist:
                bestnode = node
                bestdist = dst
        return bestnode or fallback
    
    def mincoveringdist(self, node, point, level):
        dst = dist(node, point)
        self.basictouchno += 1
        return dst if dst <= self.tree.cr * self.tree._taupow(level) else float('inf')
